...
A natural question now is whether this result holds true for other values of $k$? Below is a function that repeats this analysis for a range of values of $k$.
...
Next,  k-means with 3 clusters to each of these bootstrap samples
```{r}
...
Lastly, plot the distribution of this measure across the bootstrap samples. 
...
This time, instead of sampling the dataset with replacement to get a sample the same size as the original sample, Subsample *without replacement* to get a sample 80% of the size of the original sample.
...
Next, apply k-means with 3 clusters to each of these subsamples.
...
Again, use the same measure to summarise the clustering for each iteration.
